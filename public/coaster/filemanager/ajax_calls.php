...
if(isset($_GET['action']))
{
    switch($_GET['action'])
    {
		...
		case 'save_img':
			...
			if ($image_data === false)
			{
				response(transfm('Aviary_No_Save'), 400)->send();
				exit;
			}
		...
		case 'extract':
			...
			switch ($info['extension'])
			{
				case "zip":
					...
					{
						response(transfm('Zip_No_Extract'), 500)->send();
						exit;
					}
				...
				default:
					response(transfm('Zip_Invalid'), 400)->send();
					exit;
			}
			break;
		case 'media_preview':
		...
		case 'copy_cut':
			...
			if (is_dir($path))
			{
				...
				{
					response(sprintf(transfm('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(transfm('Copy')) : lcfirst(transfm('Cut'))), transfm('Folders')), 403)->send();
					exit;
				}
				...
				if ($copy_cut_max_size !== false && is_int($copy_cut_max_size)) {
					...
					if (($copy_cut_max_size * 1024 * 1024) < $sizeFolderToCopy) {
						response(sprintf(transfm('Copy_Cut_Size_Limit'), ($_POST['sub_action'] == 'copy' ? lcfirst(transfm('Copy')) : lcfirst(transfm('Cut'))), $copy_cut_max_size), 400)->send();
						exit;
					}
				}
				...
				if ($copy_cut_max_count !== false && is_int($copy_cut_max_count))
				{
					...
					{
						response(sprintf(transfm('Copy_Cut_Count_Limit'), ($_POST['sub_action'] == 'copy' ? lcfirst(transfm('Copy')) : lcfirst(transfm('Cut'))), $copy_cut_max_count), 400)->send();
						exit;
					}
				}
			}
			else
			{
				...
				{
					response(sprintf(transfm('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(transfm('Copy')) : lcfirst(transfm('Cut'))), transfm('Files')), 403)->send();
					exit;
				}
			}
		...
		case 'chmod':
			...
			{
				response(sprintf(transfm('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(transfm('Folders')) : lcfirst(transfm('Files'))), 403), 400)->send();
				exit;
			}
			else
			{
		...
		case 'get_lang':
			...
			{
				response(transfm('Lang_Not_Found'), 404)->send();
				exit;
			}
		...
		case 'get_file': // preview or edit
			...
			{
				response(transfm('File_Not_Found'), 404)->send();
				exit;
			}
			...
			{
				response(sprintf(transfm('File_Open_Edit_Not_Allowed'), ($sub_action == 'preview' ? strtolower(transfm('Open')) : strtolower(transfm('Edit')))), 403)->send();
				exit;
			}
