    ...
    switch($_GET['action'])
    {
        ...
        case 'rename_folder':
            if ($rename_folders){
                ...
                if (!empty($name)){
					...
					{
						response(transfm('Rename_existing_folder'), 403)->send();
						exit;
					}
                    ...
                    response(transfm('Empty_name'), 400)->send();
					exit;
                }
            }
            break;
        case 'create_file':
            if ($create_text_files === FALSE) {
                response(sprintf(transfm('File_Open_Edit_Not_Allowed'), strtolower(transfm('Edit'))), 403)->send();
				exit;
            }
            ...
            if (strpos($name, '.') === FALSE){
                response(transfm('No_Extension').' '.sprintf(transfm('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
				exit;
            }
            ...
            if (file_exists($path.$name)) {
                response(transfm('Rename_existing_file'), 403)->send();
				exit;
            }

            $content = $_POST['new_content'];
            if (@file_put_contents($path.$name, $content) === FALSE) {
                response(transfm('File_Save_Error'), 500)->send();
				exit;
            }
            else {
        ...
        case 'paste_clipboard':
            ...
            if (is_really_writable($path) === FALSE || is_really_writable($path_thumb) === FALSE){
                response(transfm('Dir_No_Write').'<br/>'.str_replace('../','',$path).'<br/>'.str_replace('../','',$path_thumb), 403)->send();
				exit;
            }

            // check if server disables copy or rename
            if (is_function_callable(($action == 'copy' ? 'copy' : 'rename')) === FALSE){
                response(sprintf(transfm('Function_Disabled'), ($action == 'copy' ? lcfirst(transfm('Copy')) : lcfirst(transfm('Cut')))), 403)->send();
				exit;
            }
        ...
        case 'chmod':
            ...
            // check perm
            if ($chmod_perm === FALSE) {
                response(sprintf(transfm('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(transfm('Folders')) : lcfirst(transfm('Files')) )), 403)->send();
				exit;
            }

            // check mode
            if (!preg_match("/^[0-7]{3}$/", $mode)){
                response(transfm('File_Permission_Wrong_Mode'), 400)->send();
				exit;
            }
            ...
            if (is_function_callable('chmod') === FALSE){
                response(sprintf(transfm('Function_Disabled'), 'chmod'), 403)->send();
				exit;
            }
