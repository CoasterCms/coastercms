...
if (!\Illuminate\Support\Facades\Auth::action('filemanager')) {
	if (!\Illuminate\Support\Facades\Auth::admin()) {
		header('Location: http://' . $_SERVER['HTTP_HOST'] . '/' . config('wf_cms::admin.url') . '/login?login_path=' . $_SERVER['REQUEST_URI']);
	}
	die('Access denied');
}
...
$config = array(
	...
	*/
	...
	'ext_misc'                                => array( 'zip', 'rar', 'gz', 'tar', 'iso', 'dmg', 'kml', 'gpx' ), //Archives
